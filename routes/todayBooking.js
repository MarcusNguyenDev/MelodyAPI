...
router.get('/BookedServices', function(req, res, next) {
    ...
    req.db.select("*").from("bookings").where("BookingDate","=",date)
    .then(bookinglist=>{
        ...
        req.db.select("*").from("bookedservices").whereIn("BookingId",bookingIdList)
...
router.get("/BookedServices/:id",(req,res,next)=>{
    ...
    req.db.select("*").from("bookedservices")
    .where("BookingId",id)
    .then(data=>res.status(200).json(data))
    .catch(err=>{
            console.log(err);
...
router.put("/BookedServices/setDone/:id",(req,res,next)=>{
    const id = req.params.id;
    req.db.from("bookedservices").update({
        Done:"YES"
    })
    .where("Id",id)
